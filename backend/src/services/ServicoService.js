...
function validarAdmin(usuario) {
  if (!usuario || usuario.role !== 'ADMIN') {
    throw new Error('Apenas administradores podem acessar este recurso');
  }
}
...
function validarDadosServico(dados) {
  ...
  if (valorBase === undefined || valorBase === null) {
    throw new Error('Valor base é obrigatório');
  }
  ...
  if (tempoPreparacaoMinutos !== undefined && tempoPreparacaoMinutos < 0) {
    throw new Error('Tempo de preparação não pode ser negativo');
  }
}
